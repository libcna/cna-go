...
| `VERIFIED_NATIVE` | Native Game event bridge | 60 isolated deliveries each of Activated, Exiting and Disposed in native stress, in order and on the owner goroutine | Game.Activated, Game.Exiting and Game.Disposed are bound to the pre-existing canonical CNA signals through cna_game_subscribe. Exactly one native subscription per event per Game; the registration handle never reaches the public API. Game.Deactivated is structurally bound and separately recorded: see game-activation-transitions. |
